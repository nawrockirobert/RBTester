#!/bin/bash
MY_PATH=$1
message="Tester le RB750 ? Appuyez sur (Entrer) pour continuer ou E pour quitter"
#declaration de variables
Ipvl=192.168.88.1
nbping=20
ping_max="20"
tempcomptearebour=10
...
function Tester {
	...
	UDPort=""
	if [ ${rtest} == 0 ] ; then
		read -p $'\n\033[32;3m'"Branchez que le PORT 2 au RB960 (c'est le cable etiqueté KO) et appuyer sur entrer"$'\n\033[0m' pff
		...
		echo ""
		read -p $'\n\033[32;3m'"Branchez les reste des ports au switch cisco noir nommé CICI"$'\n\033[0m' pff
		comptearebour
		SN=$(ssh admin@${Ipvl} ":put [/system routerboard get serial-number]")
	...
	ppping
	if [ ${Resultatest} == 0 ] || [ ${Resultaping} == 0 ] ; then
		echo -e '\033[31;3m'"test raté"'\033[0m'
		...
		echo -e '\033[32;3m'"test reussit"'\033[0m'
		echo ""
		...
		echo -e '\033[32;3m'"Vous pouvez débrancher tout les ports ethernet et laisser l'allimentation le temps que le RB750 redemarre"'\033[0m'
		echo ""
		echo ""
		message="Passez au RB750 suivant ? Appuyez sur (Entrer) pour continuer ou E pour quitter"	
	fi
}
